package com.xiaoweii.train.generator.util;
...
public class DbUtil {
    ...
    public static Connection getConnection() {
        /*
        * DbUtil是基于传统的JDBC来操作数据库, 现如今都是框架, 这里练习一下
        * */
    ...
    public static List<Field> getColumnByTableName(String tableName) throws Exception {
        ...
        Connection conn = getConnection();
        Statement stmt = conn.createStatement();
        ResultSet rs = stmt.executeQuery("show full columns from `" + tableName + "`");//反引号, 防止表名或字段名是关键字, 导致SQL报错
        if (rs != null) {
            while(rs.next()) {
                ...
                if (comment.contains("枚举")) {
                    ...
                    // 后续会将后端枚举转成前端常量, 常量的规范就是全大写, 用_连接, 从而获取枚举类型
